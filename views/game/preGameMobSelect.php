<body class="bg-[url('../public/img/WoodBG.png')] bg-cover bg-center bg-no-repeat min-h-screen overflow-hidden select-none">
    <div class='min-h-screen overflow-hidden relative'>
        <div class="bg-[url('../public/img/paperBGVertical.png')] flex-col min-h-[110vh] -rotate-3 translate-x-[-5vw] pb-[53vh] translate-y-[-20vh] drop-shadow-[0px_0px_30px_rgba(0,0,0,1)] flex align-center bg-cover bg-no-repeat absolute w-[40vw] aspect-843/1862">
            <div class="pt-[22vh] pl-[8vw] pr-[4vw] flex flex-col items-center h-full w-full">
                <div class="align-center w-full h-fit transform rotate-3 flex flex-col gap-6 min-h-0">
                    <h1 class="text-3xl sm:text-4xl md:text-6xl lg:text-9xl text-mmRed text-center font-koho tracking-koho font-bold drop-shadow-lg/80">Mobs</h1>
                    <div class="w-full flex flex-col gap-6 overflow-y-auto min-h-0  scrollbar-none">
                        <?php
                            include '../views/partials/FamilyCards/Salerno.php';
                            include '../views/partials/FamilyCards/Brennan.php';
                            include '../views/partials/FamilyCards/Ishikawa.php';
                            include '../views/partials/FamilyCards/Salazar.php';
                        ?>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>